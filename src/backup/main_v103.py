...
class LadderGame(ft.Container):
    ...
    def __init__(self, page: ft.Page):
        ...
        self.active_animations_count = 0  # <<< 실행 중인 애니메이션 카운터 추가
    ...
    def click_participants_game(self, e):
        ...
        # 카운터가 0일 때만 활성화 (일반적으로 여기서는 항상 0임)
    ...
    def start_game(self, participant_index):
        # 참가자의 버튼을 누른 경우에 배경 이미지는 default_background로 변경하기기
    ...
    async def animate_line(self, participant_index):
        ...
        # --- 카운터 감소 및 버튼 상태 확인/업데이트 ---
        self.active_animations_count -= 1
        if self.page and self.page.appbar and len(self.page.appbar.actions) > 0:
            # 모든 애니메이션이 끝났을 때만 버튼 활성화
    ...
    def new_game(self, e):
        ...
        self.vertical_lines = []
        self.current_animations = []
        self.active_animations_count = 0  # <<< 카운터 리셋
        self.log_text.value = ""
    ...
    def generate_ladder(self, e):
        """
        Regenerates the ladder view and resets all associated components to their default state.
        This method clears the canvas, resets participant name entry colors,
        result box values, and their appearance. It also re-enables all buttons
        and updates the app bar actions. The ladder's horizontal lines are
        regenerated, and the default background image is restored.
        Args:
            e: The event object triggering the ladder generation.
        Behavior:
            - Clears the log text and canvas controls.
            - Resets participant name entry colors to None.
            - Resets result boxes with default values and appearance.
            - Re-enables all buttons and app bar actions.
            - Restores the default background image.
            - Sets the win state to False.
            - Updates the UI after each major change.
        """
        if self.main_view.visible:
            self.log_text.value = ""
            self.canvas.controls.clear()
            ...
            self.vertical_lines = []
            self.current_animations = []
            self.active_animations_count = 0 # <<< 카운터 리셋 (안전 장치)
